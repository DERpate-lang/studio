  ...
  const handleAddPhoto = async () => {
    ...
    try {
      ...
      if (successfullyUploadedPhotosData.length === 0 && selectedFiles.length > 0) {
        toast({ title: "No Photos Uploaded", description: "Could not process and upload any of the selected files.", variant: "default" });
        setIsLoading(false);
        return;
      }
      ...
      if (successfullyUploadedPhotosData.length > 0) {
        const photosToInsert = successfullyUploadedPhotosData.map(pData => ({
          ...
          // id and date_added will be generated by Supabase DB
        }));
      ...
      toast({
        ...
        description: error.message || "An error occurred during the photo addition process.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleDeletePhoto = async (photo: Photo) => {
    ...
    if (confirm("Are you sure you want to delete this photo? This will remove it from storage and the database.")) {
      ...
      try {
        ...
        if (storageError) {
          // Log storage error but attempt to delete from DB anyway, or handle as critical
